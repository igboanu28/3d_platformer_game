...
namespace UnityUtils.StateMachine
{
    public abstract class StatefulEntity : MonoBehaviour
    {
        ...
        protected void At<T>(IStates from, IStates to, T condition) => stateMachine.AddTransition(from, to, condition);

        protected void Any<T>(IStates to, T condition) => stateMachine.AddAnyTransition(to, condition);
    }
}